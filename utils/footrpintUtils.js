...
const calculateBasicDetails = async (uid, basicDetails , month , year , timestamp) => {
  const {
    age,
    familyMembers,
    belowTenMembers,
...
const calculateRecycleDetails = async (uid, recycleDetails , month , year , timestamp) => {
  const {
    cansRecycle,
    plasticRecycle,
    glassRecycle,
...
const calculateTravelDetails = async (uid, data , month , year , timestamp) => {
  let footprint = 0;
  ...
  if (data.bikeCount !== 0) {
    if (data.bikeDetails.petrolBikeDistance) {
      footprint +=
        data.bikeDetails.petrolBikeDistance * travelEmissionFactors.petrolBike;
    }
    ...
    if (data.bikeDetails.hybridBikeDistance) {
      footprint +=
        data.bikeDetails.hybridBikeDistance * travelEmissionFactors.hybridBike;
    }
  }
  ...
  if (data.carCount !== 0) {
    ...
    if (data.carDetails.electricCarDistance) {
      footprint +=
        data.carDetails.electricCarDistance * travelEmissionFactors.electricCar;
    }
    ...
    if (data.carDetails.petrolCarDistance) {
      footprint +=
        (data.carDetails.petrolCarDistance / data.carDetails.petrolCarMileage) *
...
const calculateElectricityDetails = async (uid, electricityDetails , month , year , timestamp) => {
...
//energy details
const calculateEnergyDetails = async (uid, energyDetails , month , year , timestamp) => {
...
//food details
const calculateFoodDetails = async (uid, foodDetails , month , year , timestamp) => {
...
//clothing details
const calculateClothingDetails = async (uid, clothingDetails , month , year , timestamp) => {
...
const calculateExtraDetails = async (uid, extraDetails , month , year , timestamp) => {
  ...
  const monthlyVacationEmission = (vacationsCount * totalVacationEmission) / 12;
...
export const fetchAndUpdateFootprint = async uid => {
  ...
  if (doc1.exists) {
    ...
    calculateBasicDetails(uid, data , timestamp.month , timestamp.year , timestamp);
  }
  ...
  if (doc2.exists) {
    ...
    calculateRecycleDetails(uid, data , timestamp.month , timestamp.year , timestamp);
  }
  ...
  if (doc3.exists) {
    ...
    calculateTravelDetails(uid, data , timestamp.month , timestamp.year , timestamp);
  }
  ...
  if (doc4.exists) {
    ...
    calculateElectricityDetails(uid, data , timestamp.month , timestamp.year , timestamp);
  }
  ...
  if (doc5.exists) {
    ...
    calculateEnergyDetails(uid, data , timestamp.month , timestamp.year , timestamp);
  }
  ...
  if (doc6.exists) {
    ...
    calculateFoodDetails(uid, data , timestamp.month , timestamp.year , timestamp);
  }
  ...
  if (doc7.exists) {
    ...
    calculateClothingDetails(uid, data , timestamp.month , timestamp.year , timestamp);
  }
  ...
  if (doc8.exists) {
    ...
    calculateExtraDetails(uid, data , timestamp.month , timestamp.year , timestamp);
  }
};
